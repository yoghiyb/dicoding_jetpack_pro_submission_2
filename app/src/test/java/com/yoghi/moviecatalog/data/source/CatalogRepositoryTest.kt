...
import com.yoghi.moviecatalog.LiveDataTestUtil
...
class CatalogRepositoryTest {
    ...
    fun getNowPlayingMovies() {
        runBlocking {
            doAnswer {invocation ->
                (invocation.arguments[0] as RemoteDataSource.GetNowPlayingMoviesCallback).onAllMoviesReceived(listMovieResponse)
                null
    ...
    fun getMovieDetail() {
        runBlocking {
            doAnswer {invocation ->
                (invocation.arguments[1] as RemoteDataSource.GetMovieDetailCallback).onMovieDetailReceived(movieResponse)
                null
    ...
    fun getTvShowOnTheAir() {
        runBlocking {
            doAnswer { invocation ->
                (invocation.arguments[0] as RemoteDataSource.GetOnTheAirTvShowCallback).onAllTvShowsReceived(listTvShowResponse)
                null
    ...
    fun getTvShowDetail() {
        runBlocking {
            doAnswer {invocation ->
                (invocation.arguments[1] as RemoteDataSource.GetTvShowDetailCallback).onTvShowDetailReceived(tvShowResponse)
                null
